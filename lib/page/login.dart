                ...
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      ...
                      Buttons.sendButton(context, "Giriş Yap", false, () async {
                        await _login();
                      }),
                      p(context, "VEYA"),
                      Buttons.sendButton(context, "Kayıt Ol", true, () async {
                        push(context, JeaPage.register);
                      }),
                    ],
                  ),
                ),
  ...
  Future<void> _login() async {
    try {
      ...
      if (
          ...
          email.length > 6 &&
          password.length > 6) {
        var res = await ZeytinUser(zeytin).login(email, password);
        if (res.isSuccess) {
          var data = res.data;
          if (data == null) return;
